   ...
   try {
      ...
      if (!user.uCurrentGameId) {
         ...
         user = await prisma.user.update({
            where: { firebaseUid },
            data: {
               uCurrentGameId: newGameResults.ugId,
            },
         });
      }

      const gameResults = await prisma.userGame.findUnique({
         where: { ugId: user.uCurrentGameId! },
      });
...
export const addHand = async (req: AuthRequest, res: Response) => {
   try {
      ...
      if (rt.Result === "Win") {
         ...
         rt.CurrentBalance = Number((rt.CurrentBalance + rt.Bet).toFixed(2));
         rt.ProfitAmount = Number((rt.ProfitAmount + rt.Bet).toFixed(2));
      ...
      console.error("Add hand error:", error);
...
export const newGame = async (req: AuthRequest, res: Response) => {
   try {
      ...
      const profit = Number((endingBalance - startingBalance).toFixed(2));
      const isWin = profit > 0;

      const lastBalance =
         endingBalance === 0
            ? Math.max(user.uCurrentBalance ?? 0, user.defaultStartingBalance ?? 0)
            : Math.max(endingBalance, user.defaultStartingBalance ?? 0);

      const [, , newGameResults] = await prisma.$transaction([
         prisma.user.update({
            where: { firebaseUid },
            data: {
         ...
         prisma.userGame.create({
            data: {
               uId: user.uId,
               gmId: GameID || user.defaultGameId,
               mmId: MMID || user.defaultMMId,
               ugBaseUnit: BaseUnit || user.defaultBaseUnit,
...
export const undoHand = async (req: AuthRequest, res: Response) => {
   try {
      ...
      const recoveryList = lastHand?.NextHand?.RecoveryList ?? [];
      ...
      if (updatedList.length > 0) {
         const tmpNextHand = NextPredictionAndBet(updatedList, gameUserData);
...
export const skipHand = async (req: AuthRequest, res: Response) => {
   try {
      ...
      console.error("Skip hand error:", error);
      return res.status(500).json({
         success: false,
